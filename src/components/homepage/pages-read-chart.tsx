  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/lecture/stats?${params}`);
        if (!response.ok) throw new Error("Failed to fetch data");
        const data = await response.json();
        setChartData(data);
      } catch (err) {
        console.error("Error fetching pages-read chart data:", err);
            ...
            >
              ...
              <Bar
                ...
                shape={(props: unknown) => {
                  ...
                  const isZero = barProps.payload.pages === 0;
